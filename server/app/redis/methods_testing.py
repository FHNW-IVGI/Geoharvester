...
def results_ranking(redis_output, query_words_list, testing=False):
    """
    ...
    """
    if testing:
        query_results_df = redis_output
    else:
        query_results_df = json_to_pandas(redis_output)
    # initialize ranking score and the length counter
    query_results_df['score'] = 0
    query_results_df['inv_title_length'] = 200 - query_results_df['title'].str.len()#query_results_df['title'].apply(lambda x: 200 - len(x))
    ...
    if testing:
        ranked_results = query_results_df
    else:
        ranked_results = pandas_to_dict(query_results_df)
    return ranked_results

def test_ranking(query_results_df, query_words_list, sorting_fields=None):
    # initialize ranking score and the length counter
    query_results_df['score'] = 0
    query_results_df['inv_title_length'] = 200 - query_results_df['title'].str.len()#query_results_df['title'].apply(lambda x: 200 - len(x))
    query_results_df['metaquality'] = query_results_df['metaquality'].astype(int)
    ...
    if not sorting_fields:
        ...
        if query_words_list:
            for query_word in query_words_list:
                query_results_df = contains_match_scoring(query_results_df, ['title', 'keywords'], query_word, 4)
                query_results_df = contains_match_scoring(query_results_df, ['keywords_nlp'], query_word, 3)
                query_results_df = exact_match_scoring(query_results_df, ['title', 'keywords'], query_word, 6)
                query_results_df = exact_match_scoring(query_results_df, ['keywords_nlp'], query_word, 4)
                # query_results_df = exact_match_scoring(query_results_df, ['summary'], query_word, 2)
        else:
            query_results_df['score'] = 1
        #query_results_df = evaluate_metaquality(query_results_df, 25)
        query_results_df.sort_values(by=['score', 'inv_title_length'], axis=0, inplace=True, ascending=False)
    else:
        if query_words_list:
            for query_word in query_words_list:
                if len(sorting_fields[0]) != 0:
                    query_results_df = exact_match_scoring(query_results_df, sorting_fields[0], query_word, 1)
                if len(sorting_fields[1]) != 0:
                    query_results_df = contains_match_scoring(query_results_df, sorting_fields[1], query_word, 1)
        query_results_df.sort_values(by=['score'], axis=0, inplace=True, ascending=False)
    # Replace nans with empty str for a cleaner visualisation
    return query_results_df.replace(to_replace='nan', value="", regex=True)